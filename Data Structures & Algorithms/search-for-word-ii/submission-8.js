...
class Solution {
    ...
    _dfs(board, row, col, node, res) {
        ...
        if (next.word !== null) { // if word exists (hence, end of word), push to global res, and make it null to not push duplicates
            res.push(next.word);
            next.word = null; // dedup makes it so that when you cross this same path again in another dfs call, you won't add it to res
        }

        let temp = board[row][col]; // standart search w/ backtrack 
        board[row][col] = "#";
